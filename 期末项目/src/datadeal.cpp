...
int inputValid(int time){
	int y = time / 10000;
	int m = (time - (time / 10000)*10000)/100;
	int d = time - y*10000 - m*100;
    int month[12]= {31,28,31,30,31,30,31,31,30,31,30,31};
    if((m>0 && m<13) && (d>0 && d<32) && (y>1979 && y<2100)){
        if (m ==2 ){
...
bool Income::removeValue(const int & ID)
{
	...
	sql += num2str(ID);
	sql += ";";
	if (!data.modifyDataBase(sql) || !(ID >0 && ID <82) || ID-int(ID)!= 0 )
	{
		errMsg = data.getErrorMessage();
		return false;
	}
...
bool Income::setType(Type & IncomeType)
{
	...
	sql += "\");";
	if (!data.modifyDataBase(sql) || IncomeType.typeID - (int)IncomeType.typeID != 0 || IncomeType.typeID <1|| IncomeType.typeName == "Null" )
	{
		errMsg = data.getErrorMessage();
		return false;
	}
...
float Income::getSum(const int & beginTime, const int endTime) 
{
	...
	sql += num2str(endTime);
	sql += ";";
	if(!inputValid(beginTime) || !inputValid(endTime) || endTime < beginTime ){
		cout << endTime << " " << inputValid(endTime)<<endl;
		return 0;
	}
	return Sum(sql);
}
